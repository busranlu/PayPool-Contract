...
contract PayPool is ReentrancyGuard {
    ...
    event FundRetrieved(address indexed recepient, uint amount);
    ...
    /*function removeDepositAddress(address depositer) external isOwner canDepositTokens(depositer) {
        ...
        for(uint i = indexToDelete; i < depositAddresses.length; i++) {
            depositAddresses[i] == depositAddresses[i + 1];
        }
        depositAddresses.pop();
        emit AddressRemoved(depositer);
    }
    ...
    */
    ...
    function retrieveBalance() external isOwner nonReentrant {
        ...
        totalBalance = 0;
        emit FundRetrieved(owner, balance);
    }
    ...
    /*// Owner-only functions to approve/reject deposits
    function approveDeposit(uint256 index) external isOwner {
        require(index < depositHistory.length, "Invalid index");
